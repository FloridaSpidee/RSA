...
bignum* RSA::bignum_init() {
	bignum* b = (bignum*)malloc(sizeof(bignum));
	b->length = 0;
	b->capacity = BIGNUM_CAPACITY;
	b->data = (word*)calloc(BIGNUM_CAPACITY, sizeof(word));
	return b;
}

void RSA::bignum_deinit(bignum* b) {
	free(b->data);
	free(b);
}

int RSA::bignum_iszero(bignum* b) {
	return b->length == 0 || (b->length == 1 && b->data[0] == 0);
}

int RSA::bignum_isnonzero(bignum* b) {
	return !bignum_iszero(b);
}

void RSA::bignum_copy(bignum* source, bignum* dest) {
	dest->length = source->length;
	if (source->capacity > dest->capacity) {
		dest->capacity = source->capacity;
		dest->data = (word*)realloc(dest->data, dest->capacity * sizeof(word));
	}
	memcpy(dest->data, source->data, dest->length * sizeof(word));
}

void RSA::bignum_fromstring(bignum* b, char* string) {
	int i, len = 0;
...
void RSA::bignum_fromint(bignum* b, unsigned int num) {
	b->length = 1;
	if (b->capacity < b->length) {
		b->capacity = b->length;
		b->data = (word*)realloc(b->data, b->capacity * sizeof(word));
	}
	b->data[0] = num;
}

void RSA::bignum_print(bignum* b) {
	int cap = 100, len = 0, i;
...
int RSA::bignum_equal(bignum* b1, bignum* b2) {
	int i;
...
int RSA::bignum_greater(bignum* b1, bignum* b2) {
	int i;
...
int RSA::bignum_less(bignum* b1, bignum* b2) {
	int i;
	if (bignum_iszero(b1) && bignum_iszero(b2)) return 0;
...
int RSA::bignum_geq(bignum* b1, bignum* b2) {
	return !bignum_less(b1, b2);
}

int RSA::bignum_leq(bignum* b1, bignum* b2) {
	return !bignum_greater(b1, b2);
}

void RSA::bignum_iadd(bignum* source, bignum* add) {
...
void RSA::bignum_add(bignum* result, bignum* b1, bignum* b2) {
	word sum, carry = 0;
...
void RSA::bignum_isubtract(bignum* source, bignum* sub) {
	bignum* temp = bignum_init();
...
void RSA::bignum_subtract(bignum* result, bignum* b1, bignum* b2) {
	int length = 0, i;
...
void RSA::bignum_imultiply(bignum* source, bignum* mult) {
	bignum* temp = bignum_init();
...
void RSA::bignum_multiply(bignum* result, bignum* b1, bignum* b2) {
	int i, j, k;
	word carry, temp;
...
void RSA::bignum_idivide(bignum* source, bignum* div) {
...
void RSA::bignum_idivider(bignum* source, bignum* div, bignum* remainder) {
	bignum* q = bignum_init(), * r = bignum_init();
	bignum_divide(q, r, source, div);
	bignum_copy(q, source);
...
void RSA::bignum_remainder(bignum* source, bignum* div, bignum* remainder) {
	bignum* q = bignum_init();
	bignum_divide(q, remainder, source, div);
	bignum_deinit(q);
}

void RSA::bignum_imodulate(bignum* source, bignum* modulus) {
	bignum* q = bignum_init(), * r = bignum_init();
...
void RSA::bignum_divide(bignum* quotient, bignum* remainder, bignum* b1, bignum* b2) {
	...
	if (bignum_less(b1, b2)) { /* Trivial case, b1/b2 = 0 iff b1 < b2. */
		...
		if (quotient->capacity < b1->length) {
			...
			quotient->data = (word*)realloc(quotient->data, quotient->capacity * sizeof(word));
		}
		for (i = b1->length - 1; i >= 0; i--) {
		...
		if (quotient->capacity < n - m) {
			...
			quotient->data = (word*)realloc(quotient->data, (n - m) * sizeof(word));
		}
		...
		if (b1copy->length != n) {
			...
			if (b1copy->length > b1copy->capacity) {
				...
				b1copy->data = (word*)realloc(b1copy->data, b1copy->capacity * sizeof(word));
			}
			b1copy->data[n - 1] = 0;
		}
		...
		for (i = n - m - 1; i >= 0; i--) {
			...
			if (m + 1 > temp3->capacity) {
				temp3->capacity = m + 1;
				temp3->data = (word*)realloc(temp3->data, temp3->capacity * sizeof(word));
			}
			temp3->length = 0;
			for (j = 0; j <= m; j++) {
...
void RSA::bignum_modpow(bignum* base, bignum* exponent, bignum* modulus, bignum* result) {
...
void RSA::bignum_gcd(bignum* b1, bignum* b2, bignum* result) {
...
void RSA::bignum_inverse(bignum* a, bignum* m, bignum* result) {
...
int RSA::bignum_jacobi(bignum* ac, bignum* nc) {
...
int RSA::solovayPrime(int a, bignum* n) {
...
int RSA::probablePrime(bignum* n, int k) {
...
void RSA::randPrime(int numDigits, bignum* result) {
...
void RSA::randExponent(bignum* phi, int n, bignum* result) {
...
int RSA::readFile(FILE* fd, char** buffer, int bytes) {
	int len = 0, cap = BUF_SIZE, r;
	char buf[BUF_SIZE];
	*buffer = (char*)malloc(BUF_SIZE * sizeof(char));
	while ((r = fread(buf, sizeof(char), BUF_SIZE, fd)) > 0) {
		if (len + r >= cap) {
			cap *= 2;//一次只读一个BUF_SIZE，所以溢出时只要cap扩大一倍就一定能装下
...
int RSA::readString(char* source, char** buffer, int bytes) {
	int len = strlen(source), cap = BUF_SIZE, r;
	//char buf[BUF_SIZE];
	*buffer = (char*)malloc(BUF_SIZE * sizeof(char));
	if (len >= cap) {
		cap = len + 1;
...
void RSA::enCode(bignum* m, bignum* e, bignum* n, bignum* result) {
	bignum_modpow(m, e, n, result);
}

void RSA::deCode(bignum* c, bignum* d, bignum* n, bignum* result) {
	bignum_modpow(c, d, n, result);
}

bignum* RSA::encodeMessage(int len, int bytes, char* message, bignum* exponent, bignum* modulus) {
...
char* RSA::decodeMessage(int len, int bytes, bignum* cryptogram, bignum* exponent, bignum* modulus) {
	int* decoded = (int*)malloc(len * bytes * sizeof(int));
	char* cdecoded = (char*)malloc(len * bytes * sizeof(char));
	int i, j;
	...
	for (i = 0; i < len; i++) {
		...
		for (j = 0; j < bytes; j++) {
			...
			cdecoded[i * bytes + j] = (char)(decoded[i * bytes + j]);
...
RSA::RSA() {
	p = bignum_init(); q = bignum_init(); n = bignum_init();
	phi = bignum_init(); e = bignum_init(); d = bignum_init();
	bbytes = bignum_init(); shift = bignum_init();
	temp1 = bignum_init(); temp2 = bignum_init();
	srand(time(NULL));
}
...
int RSA::encodeFile(const char* path) {
	...
	printf("\n");
	encoded = encodeMessage(len, bytes, buffer, d, n);
...
void RSA::decodeFile() {
	...
	printf("\n");
	cdecoded = decodeMessage(len / bytes, bytes, encoded, e, n);
	printf("\n\nFinished RSA demonstration!");
}


bignum* RSA::encode(char* plainText) {
	len = readString(plainText, &buffer, bytes);
	encoded= encodeMessage(len, bytes, buffer, d, n);
	return encoded;
}

char* RSA::decode(char * ciphertext) {
	bignum* dst = bignum_init();
	bignum_fromstring(dst, ciphertext);
	return decodeMessage(len / bytes, bytes, dst, e, n);
}

void RSA::setN(char* n) {
	bignum* temp = bignum_init();
	bignum_fromstring(temp, n);
	RSA::n = temp;
}
void RSA::setE(char* e) {
	bignum* temp=bignum_init();
	bignum_fromstring(temp, e);
	RSA::n = temp;
}
void RSA::setD(char* d) {
	bignum* temp = bignum_init();
	bignum_fromstring(temp, d);
	RSA::n = temp;
}
